...
Conclusion: **PASS — `sand create`, `sand list`, `sand <name> status`, and `sand <name> spec` work against the real Apple backend for a stopped Sandbox VM with no Shared Folders.**
...
| A Sandbox VM can be created with no Shared Folders | PASS | Status/spec showed `sharedFolders: 0` / `sharedFolders: []`. |
...
| `sand <name> status` shows useful config/backend status | PASS | Output included name, state, image, resources, and Shared Folder count; no raw backend JSON. |
