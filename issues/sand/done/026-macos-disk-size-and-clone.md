...
- [x] Disk Size defaults to ~100GB on macOS when unspecified.
...
- Added `DiskSize` to the v1 spec as an additive macOS-only field; macOS defaults to `100GB`, Linux specs reject `disk`, and update validation rejects in-place disk edits.
